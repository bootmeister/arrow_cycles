def step(pos_x, pos_y, arrow, b_length, b_width):
    ...
    if arrow == "<":
        pos_x = make_board_range(pos_x - 1, *board_size)
    elif arrow == ">":
        pos_x = make_board_range(pos_x + 1, *board_size)
    elif arrow == "^":
        pos_y = make_board_range(pos_y - 1, *board_size)
    elif arrow == "v":
        pos_y = make_board_range(pos_y + 1, *board_size)
    print("after step xy = ", pos_x, pos_y, "arrow =", arrow)
    next_loop = input("Next Loop?")
    return pos_x, pos_y
...
while full_loop is False:
    ...
    while not (start_pos_x == cur_pos_y and start_pos_y == cur_pos_y) and (first_loop is True):
        print("cur_pos_x, cur_pos_y =",  cur_pos_x, cur_pos_y, "board[x,y]", board[cur_pos_x][cur_pos_y])
        cur_pos_x, cur_pos_y = step(cur_pos_x, cur_pos_y, board[make_board_range(cur_pos_y, *board_size,"y"):make_board_range(cur_pos_x, *board_size, "x")])
        cycle_len += 1
        if first_loop == True:
            first_loop = False
    ...
    cycle_len_map[start_pos_x][start_pos_y] = cycle_len
    print("len_map[x:y] = ", cycle_len_map[start_pos_x][start_pos_y]) 
